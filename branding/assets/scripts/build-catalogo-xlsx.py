...
PAPEL = "FFFFFF"
COBRE = "BB6A4B"
NOGAL_12 = "F2EBE6"   # ~ rgba(112,61,36,0.12) opaco sobre blanco

# --- Productos (extraídos de catalogo-productos.md) --------------------------
PRODUCTS = [
    # (Categoría, Título, Bajada, Descripción, Materialidad, Personalización,
    #  Variantes sugeridas, Foto referencia)
    ("Posavasos y mesa",
     "Posavasos Valdivia",
     ...
     "Suelto / Set 4 / Set 6",
     "20260328_162735.jpg, 20260328_123630(1).jpg"),
    ("Posavasos y mesa",
    ...
    ("Cofres y cajas",
     ...
     "20260328_122332.jpg, 20260328_123021.jpg"),
    ...
    ("Marcapáginas",
     ...
     "20260531_125703.jpg, 20260531_130023.jpg, 20260531_130419.jpg"),
...
c.value = ("Edita las columnas grises (dimensiones, plazo, precio, foto definitiva, visible, orden). "
          "Las columnas azules son la propuesta del taller (título, bajada, descripción) — modifícalas si no calzan con tu voz.")
...
HEADERS = [
    ("Categoría",            24, "ref"),
    ("Título",               34, "edit_blue"),
    ("Bajada (lista)",       52, "edit_blue"),
    ("Descripción (ficha)",  60, "edit_blue"),
    ("Materialidad",         48, "edit_blue"),
    ("Personalización",      48, "edit_blue"),
    ("Variantes sugeridas",  38, "edit_blue"),
    ("Foto referencia",      32, "ref"),
    ("Dimensiones (mm)",     22, "edit_gray"),
    ("Plazo (días)",         14, "edit_gray"),
    ("Precio CLP",           16, "edit_gray"),
    ("Foto definitiva",      26, "edit_gray"),
    ("Mostrar en sitio (sí/no)", 22, "edit_gray"),
    ("Orden",                10, "edit_gray"),
]
...
BLUE_TINT = "E8EEF7"   # azul muy clarito (campos editables del taller)
GRAY_TINT = "F4F4F4"   # gris claro (campos a completar por el cliente)
REF_TINT  = "FFFFFF"   # ref / categoría
...
# Freeze panes (encabezado + Categoría + Título)
ws.freeze_panes = "C4"

# Filtro
last_row = HEAD_ROW + len(PRODUCTS)
ws.auto_filter.ref = f"A{HEAD_ROW}:{get_column_letter(len(HEADERS))}{last_row}"
...
INSTR = [
    ...
    ("Categoría y Foto referencia son para que sepas de qué producto hablamos. No es necesario editarlas, pero puedes si crees que el producto va en otra categoría.", "p"),
    ("", "spacer"),
    ("Cuando termines", "h2"),
    ...
    ("No: \"calidad premium\", \"plazos competitivos\", \"nuestro equipo\", emojis 🥰💖🎁.", "li"),
...
# Mover Instrucciones al principio del libro
wb.move_sheet("Instrucciones", offset=-1)

# === Guardar ================================================================
out = "/root/lasercrafts/branding/catalogo-productos.xlsx"
wb.save(out)
print(f"OK: {out}  ({len(PRODUCTS)} productos)")
